...
def parse_arguments():
    ...
    parser.add_argument(
        "--size", type=int, default=20, help="Size of the maze (default: 20)"
    )
    parser.add_argument(
        "--dirt", type=int, default=10, help="Number of dirt particles (default: 10)"
    )
    parser.add_argument(
        "--seed", type=int, default=None, help="Random seed for reproducible worlds"
    )
    parser.add_argument(
        "--maze",
        choices=["default", "simple", "office", "caves"],
        default="default",
        help="Maze type to use (default: default)",
    )
    parser.add_argument(
        "--search",
        choices=["bfs", "dfs", "astar", "random"],
        default="bfs",
        help="Search method to use (default: bfs)",
    )
    parser.add_argument(
        "--no-gui", action="store_true", help="Run without graphical interface"
    )
    parser.add_argument(
        "--cell-size",
        type=int,
...
def main():
    ...
    print(
        f"[bold]Created world: [/bold][white] {args.size}x{args.size}, {args.dirt} dirt particles"
    )
...
def run_with_gui(world: World, agent: IntelligentVacuumAgent, cell_size: int):
    ...
    print()
    print(
        "The agent will start moving automatically using the selected search algorithm."
    )
