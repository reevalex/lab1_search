...
class State:
    ...
    def next_state(self, pit):
        ...
        if (
            pits[last_pit] == 1
            and Kalah.pit_to_player(last_pit) == self.current_player
            and pits[Kalah.opposing_pit(last_pit)] != 0
            and last_pit not in [PLAYER_0_STORE, PLAYER_1_STORE]
        ):
            pits[self.current_players_store] += (
                pits[Kalah.opposing_pit(last_pit)] + pits[last_pit]
            )
        ...
        if not (
            (self.current_player == 0 and last_pit == PLAYER_0_STORE)
            or (self.current_player == 1 and last_pit == PLAYER_1_STORE)
        ):
...
class AI(Player, ABC):
    @staticmethod
    @abstractmethod
    def best_move(current_state: State, objective: Objective) -> Union[int, None]:
        pass
...
class MainScene(Scene):
    def initialize(self):
        ...
        title = TextBox(
            WIDTH / 2, HEIGHT / 2, "Kalah", "center", font=pygame.font.Font(None, 72)
        )
        ...
        button_hvai = Button(
            ...
            50,
            game.start_kalah,
            {"player0": Human(), "player1": game.ai1()},
            button_text="H vs AI",
        )
        ...
        button_aivai = Button(
            ...
            50,
            game.start_kalah,
            {"player0": game.ai0(), "player1": game.ai1()},
            button_text="AI vs AI",
        )
...
class KalahScene(Scene):
    ...
    def draw(self):
        ...
        board_length = PITS_PER_PLAYER * (
            PIT_RADIUS * 2.5
        )  # Also accounts for the gaps between the pits
        ...
        pygame.draw.rect(
            game.screen,
            PIT_COLOR,
            (left_store_anchor - 55, 245, 60, 20),
            border_radius=5,
        )
        pygame.draw.rect(
            game.screen,
            PIT_COLOR_DARK,
            (left_store_anchor - 55, 145, 60, 110),
            border_radius=5,
        )
        pygame.draw.rect(
            game.screen,
            PIT_COLOR_LIGHT,
            (left_store_anchor - 50, 150, 50, 100),
            border_radius=5,
        )
        pygame.draw.rect(
            game.screen,
            PIT_COLOR,
            (left_store_anchor - 50, 150, 50, 10),
            border_radius=5,
        )
        ...
        pygame.draw.rect(
            game.screen,
            PIT_COLOR,
            (right_store_anchor - 5, 245, 60, 20),
            border_radius=5,
        )
        pygame.draw.rect(
            game.screen,
            PIT_COLOR_DARK,
            (right_store_anchor - 5, 145, 60, 110),
            border_radius=5,
        )
        pygame.draw.rect(
            game.screen,
            PIT_COLOR_LIGHT,
            (right_store_anchor, 150, 50, 100),
            border_radius=5,
        )
        pygame.draw.rect(
            game.screen, PIT_COLOR, (right_store_anchor, 150, 50, 10), border_radius=5
        )

        game.screen.blit(
            game.font.render(str(kalah.state.pits[PLAYER_0_STORE]), True, TEXT_COLOR),
            (right_store_anchor + 10, 180),
        )
        game.screen.blit(
            game.font.render(str(kalah.state.pits[PLAYER_1_STORE]), True, TEXT_COLOR),
            (left_store_anchor - 40, 180),
        )
        ...
        pygame.draw.rect(
            game.screen,
            BOARD_COLOR_DARK,
            (left_store_anchor, 110, board_length, 210),
            border_radius=20,
        )
        pygame.draw.rect(
            game.screen,
            BOARD_COLOR,
            (left_store_anchor, 100, board_length, 200),
            border_radius=20,
        )
...
class Game(metaclass=Singleton):
                ...
                if (kalah.current_player == 0 and isinstance(self.player0, AI)) or (
                    ...
                    if self.next_move == -1:
                        if kalah.current_player == 0:
                            self.next_move = self.player0.best_move(
                                kalah.state, Objective.MAX
                            )
                        else:
                            self.next_move = self.player1.best_move(
                                kalah.state, Objective.MIN
                            )

                    print(
                        f"[{DCOL_AI}][AI] [white]Move found in {(time.perf_counter() - ai_thinking_starts) * 1000:0.1f}ms"
                    )

                    # Wait some time to let the player see their move and its outcome
                    if self.last_move_time * 1000 + AI_WAIT_TIME >= time.time() * 1000:
                        time_to_wait = (
                            self.last_move_time * 1000
                            + AI_WAIT_TIME
                            - time.time() * 1000
                        )
    ...
    def make_victory_popup(self):
        ...
        game.add_object(
            TextBox(WIDTH - 10, HEIGHT - 10, victory_text, anchor="bottomright")
        )
...
class Kalah(metaclass=Singleton):
    ...
    def initialize(self):
        ...
        for i in range(PITS_PER_PLAYER):
            pit1 = BoardPit(
                50 + left_board_edge + i * (PIT_RADIUS * 2.5),
                250,
                PIT_RADIUS,
                self.try_move,
                args={"pit": i},
            ...
            pit2 = BoardPit(
                50 + left_board_edge + (PITS_PER_PLAYER - 1 - i) * 100,
                150,
                PIT_RADIUS,
                self.try_move,
...
class BoardPit(Clickable):
    ...
    def draw(self):
        ...
        game.screen.blit(
            game.font.render(str(self.seeds), True, TEXT_COLOR),
            (self.x - FONT_SIZE / 4, self.y - FONT_SIZE / 4),
        )
